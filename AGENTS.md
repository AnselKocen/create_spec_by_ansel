...
4. Ask the key setup questions if they are not already answered:
   ...
   - Image generation timing defaults to `first-run-cache`: the app shows a themed asset preparation flow on first launch, checks IndexedDB first, reads cached Blobs on cache hit, creates session object URLs, loads/decodes images, generates only cache-miss images, stores image Blobs in IndexedDB, and reuses cached Blobs on later launches without regenerating. If image generation returns a URL, the app must confirm it is readable in the current page and convertible to a real image Blob before caching or marking it ready. Required images gate the normal image experience, not entry into the product itself. If any required or optional image generation, URL-to-Blob conversion, loading, or decoding fails, or asset preparation times out / is canceled by the user, the default fallback allows the user to skip asset preparation and enter an explicit no-image fallback experience while failed assets remain `failed` / `ready = false` with retry available. Ask whether any required images must be `build-time` exceptions, or whether a `hybrid` split is needed.
...
- Image generation is an asset gate, not an optional visual enhancement. If a product requires any real bitmap asset, default to `first-run-cache`: first app launch checks IndexedDB before generating; cache hit reads the cached Blob, creates a session object URL, loads/decodes it, and must not call the image API; cache miss shows a themed asset preparation page, generates only missing images, converts the result to a Blob, stores image Blobs in IndexedDB, creates object URLs, loads/decodes them, then enters the normal image experience after required images are ready. Required images gate the normal image experience, not entry into the product itself. Later launches in the same browser/origin reuse cached Blobs without regenerating.
...
- For `first-run-cache` or the first-run branch of `hybrid`, the final app must include a themed first-run asset preparation page, check IndexedDB first, generate only cache-miss images, store image Blobs in IndexedDB, create session object URLs from IndexedDB Blobs, load/decode images before setting `ready = true`, and reuse cached Blobs on later launches without calling the image API.
- The final spec must include a pre-generation/preload gate: all `required = true` images must reach `generated + cached/cache_hit + loaded + decoded + ready` before the normal image experience starts; the normal image experience must not generate, request, or wait for required images page-by-page. If this gate cannot be passed because any required or optional image fails, times out, or the user cancels asset preparation, the user must be allowed to enter an explicit no-image fallback experience.
...
- If image generation, URL-to-Blob conversion, loading, or decoding fails for any required or optional image, or asset preparation times out / is canceled by the user, the app must allow the user to skip asset preparation and enter an explicit no-image fallback experience. Failed assets must remain `failed` / `ready = false`, expose retry, and must not be replaced by fake generated placeholders.
