...
- The final spec must include a pre-generation/preload gate: all `required = true` images must reach `generated + cached/cache_hit + loaded + decoded + ready` before the core experience starts; the core experience must not generate, request, or wait for required images page-by-page.
