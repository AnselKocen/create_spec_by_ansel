...
- The final spec must include a pre-generation/preload gate: all `required = true` images must reach `generated + cached/cache_hit + loaded + decoded + ready` before the normal image experience starts; the normal image experience must not generate, request, or wait for required images page-by-page.
...
- If image generation, URL-to-Blob conversion, loading, or decoding fails, the app must allow the user to skip asset preparation and enter an explicit no-image experience. Failed assets must remain `failed` / `ready = false`, expose retry, and must not be replaced by fake generated placeholders.
