...
当 `IMAGE_GENERATION_TIMING` 为 `first-run-cache` 或 `hybrid` 时，最终 app 必须包含一个主题化资产准备页。凡是必需图片没有 ready，用户不得进入依赖这些图片的核心体验。
...
- 单张失败时可重试并显示原因。
- 多次失败后保留错误状态和重试入口，并说明缺失的资产 ID / 用途。
- 如果必需图片无法生成，停留在资产准备页或错误页，由用户处理生图能力问题。
...
- 进入核心体验前，每个必需图片都必须有 `IMAGE_ASSET_RUNTIME_STATE.ready = true`。
...
在生图能力可用之前，不得交付声称完成的最终成品。不可用 CSS/SVG/Canvas/emoji 冒充生图结果，也不可以把“待生图”占位当作通过验收。
